...
function BasicMedia({asset,onSelect,innerRef,active}){
 const [texture,setTexture]=useState(null);
 useEffect(()=>{
  let disposed=false,video=null,objectTexture=null,loadedImage=null;
  if(asset.kind==='video' && active){
   video=document.createElement('video');
   video.src=asset.url; video.muted=true; video.loop=true; video.playsInline=true; video.preload='auto';
   const ready=()=>{if(disposed)return;objectTexture=new THREE.VideoTexture(video);objectTexture.colorSpace=THREE.SRGBColorSpace;objectTexture.minFilter=THREE.LinearFilter;objectTexture.magFilter=THREE.LinearFilter;objectTexture.generateMipmaps=false;setTexture(objectTexture);video.play().catch(()=>{});};
   video.addEventListener('loadeddata',ready,{once:true}); video.load();
  }else if(asset.kind==='image'){
   const loader=new THREE.TextureLoader();
   loader.load(asset.url,t=>{if(disposed){t.dispose();return;}loadedImage=t;t.colorSpace=THREE.SRGBColorSpace;t.minFilter=THREE.LinearFilter;t.magFilter=THREE.LinearFilter;t.generateMipmaps=false;setTexture(t);});
  }
  return()=>{disposed=true;if(video){video.pause();video.removeAttribute('src');video.load();}if(objectTexture)objectTexture.dispose();if(loadedImage)loadedImage.dispose();setTexture(null);};
 ...
 return <mesh ref={innerRef} position={asset.position} rotation={asset.rotation} scale={asset.scale} onClick={e=>{e.stopPropagation();onSelect(asset.id)}}>
   ...
   <meshStandardMaterial color={visual&&texture?'#ffffff':'#465064'} map={texture||null} metalness={.05} roughness={.7} side={THREE.DoubleSide}/>
 </mesh>
}

function SceneObject({asset,onSelect,selected,transformMode,onTransform}){
 const ref=useRef();
 const child=asset.kind==='model'?<Model asset={asset} onSelect={onSelect} innerRef={ref}/>:asset.kind==='primitive'?<Primitive asset={asset} onSelect={onSelect} innerRef={ref}/>:<BasicMedia asset={asset} onSelect={onSelect} innerRef={ref} active={selected}/>;
 if(!selected)return child;
 return <TransformControls mode={transformMode} onMouseUp={()=>{if(ref.current)onTransform(asset.id,ref.current)}}>{child}</TransformControls>
}

function Viewport({assets,selected,setSelected,transformMode,onTransform}){
 const visible=assets.slice(0,32);
 return <Canvas camera={{position:[4,3,6],fov:45,near:.1,far:1000}} dpr={[1,1.25]} gl={{antialias:false,powerPreference:'high-performance'}} onCreated={({gl})=>{gl.outputColorSpace=THREE.SRGBColorSpace;gl.setPixelRatio(Math.min(window.devicePixelRatio||1,1.25));}} onPointerMissed={()=>setSelected(null)}>
   <color attach="background" args={['#080a0f']}/>
   <ambientLight intensity={1}/><directionalLight position={[4,6,4]} intensity={1.5}/>
   <Grid args={[16,16]} cellSize={1} cellThickness={.35} sectionSize={4} sectionThickness={.7} fadeDistance={18} fadeStrength={1}/>
   <OrbitControls makeDefault enableDamping={false} enablePan enableZoom/>
   <group>{visible.map(a=><SceneObject key={a.id} asset={a} selected={selected===a.id} onSelect={setSelected} transformMode={transformMode} onTransform={onTransform}/>)}</group>
   {!assets.length&&<mesh><boxGeometry args={[1.5,1.5,1.5]}/><meshStandardMaterial wireframe/></mesh>}
 </Canvas>
}

function App(){
 const [active,setActive]=useState('Media'),[playing,setPlaying]=useState(false),[assets,setAssets]=useState([]),[selected,setSelected]=useState(null),[project,setProject]=useState('Untitled Project'),[transformMode,setTransformMode]=useState('translate');
 const input=useRef();
