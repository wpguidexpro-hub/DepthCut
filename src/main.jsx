...
function App(){
 ...
 const setMediaTime=t=>{setTime(Math.max(0,t));if(video.current&&sel?.kind==='video')video.current.currentTime=Math.max(0,t);if(audio.current&&sel?.kind==='audio')audio.current.currentTime=Math.max(0,t)};
 ...
 const splitSelected=()=>{if(!sel)return;const cut=Math.max(.05,Math.min(clipDuration(sel)-.05,time));if(cut<=.05||cut>=clipDuration(sel)-.05)return;const firstEnd=(sel.start||0)+cut;const second={...sel,id:crypto.randomUUID(),name:`${sel.name} (2)`,start:firstEnd,end:sel.end||sel.duration||5};setAssets(a=>a.flatMap(x=>x.id===sel.id?[{...x,end:firstEnd},second]:[x]));setSelected(second.id);setTime(0)};
 ...
 return <div className="app">
  ...
  <footer className="timeline"><div className="timelineHead"><div><b>Timeline</b><span>{fmt(time)} / {fmt(total)}</span></div><div className="timelineTools"><button title="Split selected clip" onClick={splitSelected}><Scissors/></button></div></div><div className="timelineBody"><div className="trackLabels"><span>🎬 Video</span><span>🖼 Overlay</span><span>🔊 Audio</span></div><div className="trackScroll"><div className="ruler">{[0,5,10,15,20,25,30].map(x=><span key={x}>{fmt(x)}</span>)}</div><div className="tracks" onClick={seek}>
    {assets.filter(a=>a.kind==='video').map(a=><TimelineClip key={a.id} a={a} selected={selected===a.id} onSelect={choose} beginDrag={beginDrag} total={total} remove={remove} trimClip={trimClip} />)}
    {assets.filter(a=>a.kind==='image'||a.kind==='text').map(a=><TimelineClip key={a.id} a={a} selected={selected===a.id} onSelect={choose} beginDrag={beginDrag} total={total} remove={remove} trimClip={trimClip} overlay />)}
    {assets.filter(a=>a.kind==='audio').map(a=><TimelineClip key={a.id} a={a} selected={selected===a.id} onSelect={choose} beginDrag={beginDrag} total={total} remove={remove} trimClip={trimClip} audio />)}
